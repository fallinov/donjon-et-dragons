...
<template>
  <header class="print-hero relative mb-10 motion-safe:animate-rise">
    ...
    <div class="xl:hidden">
      ...
      <!-- Vitals sous la carte (CA, Initiative, Vitesse) -->
      <dl class="grid grid-cols-3 gap-2 mt-4">
        <div
          v-for="vital in character.vitals.filter(v => v.label !== 'Points de vie')"
          :key="vital.label"
          class="border border-gold/40 bg-charcoal/60 px-3 py-2 text-center"
        >
          <dt class="font-display text-xs tracking-wider-3 text-gold uppercase">{{ vital.label }}</dt>
          <dd class="font-display text-xl text-parchment mt-1">
            {{ vital.value }}<span v-if="vital.unit" class="text-xs text-parchment-mute ml-0.5">{{ vital.unit }}</span>
          </dd>
        </div>
      </dl>
    </div>
